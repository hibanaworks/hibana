//! Route projection WireFirst test with a shared local prefix (compile-pass).
//!
//! Passive role performs the same local self-send prefix in both arms, then
//! distinguishes by recv label. The local prefix is identical, so dispatch
...
// Shared local prefix: Passive self-send (no wire)
type LocalPrefixMsg = g::Msg<7, ()>;

// Data messages with distinct labels for passive observer
type DataMsgA = g::Msg<10, ()>;
type DataMsgB = g::Msg<20, ()>;

// Arm0: self-send control, Passive local prefix, then Controller→Passive DataMsgA
type Arm0Steps = StepCons<
    SendStep<Controller, Controller, ControlArm0>,
    StepCons<
        SendStep<Passive, Passive, LocalPrefixMsg>,
        StepCons<SendStep<Controller, Passive, DataMsgA>, StepNil>,
    >,
>;

// Arm1: self-send control, Passive local prefix, then Controller→Passive DataMsgB
...
const ARM0: g::Program<Arm0Steps> = g::seq(
    ...
    g::send::<Passive, Passive, LocalPrefixMsg, 0>()
