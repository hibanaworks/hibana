                            ...
                            |controller| {
                                block_on_async(async {
                                    ...
                                    let branch = worker
                                        .offer()
                                        .await
                                        .expect("split worker offer must select routed payload");
                                    assert_eq!(branch.label(), ROUTE_RIGHT_PAYLOAD_LOGICAL);
                                    let payload = branch
                                        .decode::<Msg<ROUTE_RIGHT_PAYLOAD_LOGICAL, u8>>()
                                        .await
                                        .expect("split worker decode must commit route arm");
                                    assert_eq!(payload, 9);
                                });
                            },
                        );
                            ...
                            |controller| {
                                block_on_async(async {
                                    ...
                                    worker
                                        ...
                                        .send(&1u8)
                                        .await
                                        .expect("split tail send must progress after route decode");
...
fn in_place_split_kits_one_endpoint_allow_route_tail_send() {
    with_fixture(|_clock, controller_tap, slab| {
        ...
        let (controller_slab, worker_slab) = slab.split_at_mut(512 * 1024);
        let controller_storage = Box::leak(Box::new(SessionKitStorage::<
            TestTransport,
            DefaultLabelUniverse,
            hibana::integration::runtime::CounterClock,
            1,
        >::uninit()));
        let worker_storage = Box::leak(Box::new(SessionKitStorage::<
            TestTransport,
            DefaultLabelUniverse,
            hibana::integration::runtime::CounterClock,
            1,
        >::uninit()));
        let controller_guard = controller_storage.init();
        let worker_guard = worker_storage.init();
        let controller_kit = unsafe {
            // SAFETY: both storage owners are leaked for this test body and the
            // guards stay live until all endpoint TLS slots are consumed below.
            &*((&*controller_guard)
                as *const hibana::integration::SessionKit<
                    'static,
                    TestTransport,
                    DefaultLabelUniverse,
                    hibana::integration::runtime::CounterClock,
