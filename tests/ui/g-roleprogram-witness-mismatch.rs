...
fn expect_program_a(_program: RoleProgram<'static, 0, ProgramASteps>) {}

fn main() {
    let _expected: RoleProgram<'static, 0, ProgramASteps> = project(&PROGRAM_A);
    let wrong: RoleProgram<'static, 0, ProgramBSteps> = project(&PROGRAM_B);
