...
fn public_docs_use_canonical_surface_paths() {
    ...
    for source in [&readme, &lib_rs] {
        for forbidden in [
            ...
            "RouteBranch::{label, decode, into_endpoint}",
            "Endpoint::send_direct",
            "Endpoint::recv_direct",
            "SessionKit::init_in_place",
            "SessionKit::enter_into",
            "let (endpoint, _) = endpoint.flow",
            "received.1",
            "static mut TAP_BUF",
            "static mut SLAB",
        ] {
    ...
    for required in [
        ...
        "lower-layer storage is slab-backed, not heap-backed",
        "`no_alloc` oriented in",
        "reserves 96 KiB free",
        "flash <= 768 KiB",
        "static SRAM <= 48 KiB",
        "kernel stack <= 24 KiB",
        "peak SRAM <= 96 KiB",
        "peak stack upper-bound",
