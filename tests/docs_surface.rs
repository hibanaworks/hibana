...
fn readme_stays_self_contained_and_hibana_scoped() {
    ...
    for required in [
        ...
        "one choreography for up to 256 roles",
        "Role IDs cover `0..=255`",
        "does not reserve a 256-entry role table",
        "not the original choreography as one global protocol",
...
fn readme_covers_role_protocol_and_integration_without_a_second_guide() {
    ...
    for required in [
        ...
        "not an equivalent encoding of one global choreography across those sessions",
