...
if rg -n "${NAMED_UNDERSCORE_PATTERN}" README.md tests >/dev/null; then
  echo "underscore discard violation: docs, tests, and fixtures must consume values explicitly" >&2
...
LAYER_WORD='sh''im'
DISALLOWED_BYPASS_PATTERN="\\b(_${OLD_WORD}|_${MODE_WORD}|_${ALT_WORD}|_${RECOVERY_WORD}|_${GUESS_WORD}|_${LAYER_WORD}|${OLD_WORD}_|${MODE_WORD}_|${ALT_WORD}_|${RECOVERY_WORD}_|${GUESS_WORD}_|${LAYER_WORD}_)\\b"
if rg -n "${DISALLOWED_BYPASS_PATTERN}" \
  src tests README.md >/dev/null; then
  echo "underscore discard violation: forbidden bypass vocabulary detected" >&2
  rg -n "${DISALLOWED_BYPASS_PATTERN}" \
