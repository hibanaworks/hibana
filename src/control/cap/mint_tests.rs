use super::{CapError, CapHeader, CapShot, ControlOp, ControlPath, ControlToken};
use crate::{
    control::{
        cap::resource_kinds::LoopDecisionHandle,
        types::{Lane, SessionId},
    },
    g::control::LoopContinue,
...
const EXACT_LENGTH_TAG: u8 = 0x7E;
...
fn endpoint_header(token: &ControlToken) -> Result<CapHeader, CapError> {
    decode_canonical_endpoint_identity(token).map(|(header, _handle)| header)
}

fn endpoint_identity(token: &ControlToken) -> Result<EndpointHandle, CapError> {
...
fn token_from_wire(
    ...
    ControlToken::from_raw_bytes(bytes)
}
...
fn endpoint_token_with_mutated_header(
    ...
    mutate(&mut header);
    token_from_wire([0u8; super::CAP_NONCE_LEN], header)
}

#[test]
fn control_token_reads_descriptor_header_and_handle_bytes() {
    ...
    let token = token_from_wire([0u8; CAP_NONCE_LEN], header);
...
fn cap_header_decode_rejects_unknown_atomic_fields() {
    ...
    CapHeader::new(
        ...
        Lane::new(3),
        1,
        <LoopContinue as super::LocalControlKind>::TAG,
        <LoopContinue as super::LocalControlKind>::OP,
        ControlPath::Local,
        CapShot::One,
        <LoopContinue as super::LocalControlKind>::SCOPE,
        0,
        1,
        2,
...
#[test]
fn malformed_control_token_header_fails_closed() {
...
fn endpoint_header_rejects_noncanonical_decodable_fields() {
    fn mutate_tag(header: &mut [u8; super::CAP_HEADER_LEN]) {
        header[7] = <LoopContinue as super::LocalControlKind>::TAG;
    }
