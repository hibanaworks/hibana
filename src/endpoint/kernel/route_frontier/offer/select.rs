    ...
    B: BindingSlot + 'r,
{
    ...
    #[inline]
    pub(super) fn record_scope_ack(&mut self, scope_id: ScopeId, token: RouteDecisionToken) {
    ...
    #[inline]
    pub(super) fn mark_scope_ready_arm(&mut self, scope_id: ScopeId, arm: u8) {
        self.endpoint
    ...
    #[inline]
    pub(super) fn mark_scope_ready_arm_from_frame_label(
        &mut self,
        scope_id: ScopeId,
        lane: u8,
        frame_label: u8,
    ...
    #[inline]
    pub(super) fn mark_scope_ready_arm_from_binding_frame_label(
        &mut self,
        scope_id: ScopeId,
        lane: u8,
        frame_label: u8,
    ...
    pub(super) fn await_static_passive_progress(
        ...
        if !ingress.has_transport() {
            return self.await_transport_payload_for_offer_lane(selection.offer_lane, ingress, cx);
        }
        Poll::Ready(Ok(()))
    }
