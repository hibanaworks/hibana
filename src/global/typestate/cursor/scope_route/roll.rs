...
impl EventCursor {
    ...
    fn roll_scope_contains_index(&self, scope: ScopeId, idx: usize) -> bool {
        if matches!(scope.kind(), crate::global::const_dsl::ScopeKind::Loop)
            && let Some(row) = self.machine().loop_scope_row(scope)
        {
            return row.start() <= idx && idx < row.end();
        }
        if self.route_scope_linger(scope) {
            return self
                .route_linger_scope_for_index(idx)
    ...
    fn first_loop_lane_event_index(&self, scope: ScopeId, lane: u8) -> Option<usize> {
        if let Some(row) = self.machine().loop_scope_row(scope) {
    ...
    fn roll_scope_events_complete(
        ...
        let mut found = false;
        let (mut idx, end) = if let Some(row) = self.machine().loop_scope_row(scope) {
            (row.start(), row.end())
        } else {
    ...
    fn complete_loop_scope_for_index(
        &self,
        idx: usize,
        mut selected_arm_for_scope: impl FnMut(ScopeId) -> Option<u8>,
    ) -> Option<ScopeId> {
        ...
        while let Some((scope, row)) = self.machine().loop_scope_row_by_slot(slot) {
            if row.start() <= idx
                && idx < row.end()
                && row.len() >= best_len
    ...
    fn complete_roll_scope_for_index(
        ...
        self.complete_loop_scope_for_index(idx, &mut selected_arm_for_scope)
            .or_else(|| {
                let scope = self.route_linger_scope_for_index(idx)?;
    ...
    pub(crate) fn roll_reentry_index_for_label(
        ...
        while idx < self.local_steps_len() {
            ...
            if label == target_label && self.first_loop_lane_event_index(scope, lane) == Some(idx) {
                return Some(idx);
            }
            idx += 1;
        }
    ...
    pub(crate) fn roll_reentry_event_allows_index(
        ...
        self.first_loop_lane_event_index(scope, lane) == Some(idx)
    }
    ...
    pub(crate) fn roll_reentry_scope_for_step(
        ...
        let scope = self
            .complete_loop_scope_for_index(idx, |_| None)
            .or_else(|| self.route_linger_scope_for_index(idx))?;
        (self.first_loop_lane_event_index(scope, lane) == Some(idx)).then_some(scope)
    }
