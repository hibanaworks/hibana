...
fn retain_large_choreography_symbols() {
    ...
    let _ = linear_program::run
        as fn(&mut localside::ControllerEndpoint<'_>, &mut localside::WorkerEndpoint<'_>);
    let _ = linear_program::controller_program as fn() -> crate::runtime::program::RoleProgram<0>;
    let _ = fanout_program::run
        as fn(&mut localside::ControllerEndpoint<'_>, &mut localside::WorkerEndpoint<'_>);
    let _ = fanout_program::controller_program as fn() -> crate::runtime::program::RoleProgram<0>;
...
struct RuntimeShapeMetrics {
    slab_bytes: usize,
    session_kit_storage_bytes: usize,
    resident_prefix_bytes: usize,
    tap_ring_bytes: usize,
...
struct LargeChoreographyTransport {
    state: &'static LocalKey<UnsafeCell<LargeChoreographyTransportState>>,
}
...
impl LargeChoreographyTransport {
    ...
    fn with_state<R>(&self, f: impl FnOnce(&mut LargeChoreographyTransportState) -> R) -> R {
...
fn with_large_choreography_workspace<R>(
    f: impl FnOnce(&'static mut [u8; HOST_MEASURE_SLAB_BYTES]) -> R,
) -> R {
    TEST_SLAB.with(|slab| unsafe {
        let slab = &mut *slab.get();
        LargeChoreographyTransport::shared().reset_state();
        slab.fill(0);
        f(&mut *(slab as *mut [u8; HOST_MEASURE_SLAB_BYTES]))
    })
}
...
fn run_attached_shape(
    ...
    controller_program: fn() -> crate::runtime::program::RoleProgram<0>,
    worker_program: fn() -> crate::runtime::program::RoleProgram<1>,
    ...
    with_large_choreography_workspace(|slab| {
        let full_slab_len = slab.len();
        ...
        assert!(
            transport.with_state(|state| state.roles.iter().all(|role| role.queue.len == 0)),
        ...
        let runtime_snapshot = {
            ...
            let resident_prefix_bytes = metric_sub(full_slab_len, runtime_slab_len);
...
fn assert_large_choreography_runtime_metrics(shape: &'static str, metrics: RuntimeShapeMetrics) {
    assert!(
    ...
    println!(
        "large-choreography-runtime shape={shape} slab_bytes={} session_kit_storage_bytes={} resident_prefix_bytes={} tap_ring_bytes={} sidecar_scratch_high_water_bytes={} image_frontier_bytes={} frontier_workspace_bytes={} live_endpoint_bytes={} peak_live_slab_bytes={} localside_peak_stack_bytes={} peak_stack_bytes={}",
        metrics.slab_bytes,
        metrics.session_kit_storage_bytes,
        metrics.resident_prefix_bytes,
        metrics.tap_ring_bytes,
